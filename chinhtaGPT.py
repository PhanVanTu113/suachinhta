...
if uploaded_file:
    with st.spinner("📑 Đang xử lý tệp..."):
        ...
        if doc:
            ...
            st.markdown(f'<a class="download-btn" href="data:application/pdf;base64,{b64_pdf_corrected}" download="{filename}_da_sua.pdf">📥 Tải file PDF đã sửa</a>', unsafe_allow_html=True)
        else:
